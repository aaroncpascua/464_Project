...
def main(argv):
  ...
  for testNum in range(len(qList)):
    ...
    b = np.complex128(random_unitary([N,1], True))
    c = np.copy(b)
    cc = np.copy(b)
    ...
    def float_to_hex(f):
          return '{:08x}'.format(unpack('<I', pack('<f', f))[0])

    def double_to_hex(f):
          return '{:016x}'.format(unpack('<Q', pack('<d', f))[0])
    ...
    for i,A in enumerate(a) :
      for j,v in enumerate(A.flatten()):
        ...
        aStr+=" @{:08X} ".format(a_address+i*N*N+j) + double_to_hex(v.real.item()) + double_to_hex(v.imag.item()) + "\n"
